...
int _mossNRsvdcmp(register double *a, const int m, const int n, double *w, register double *v)
{
	...
	for (i=1;i<=n;i++) {
		...
		g=s=scale=0.0;
		if (i <= m) {
			for (k=i;k<=m;k++) scale += fabs(a[k*nc+i]);
			if (scale) {
				for (k=i; k<=m; k++) {
					a[k*nc+i] /= scale;
					s += a[k*nc+i]*a[k*nc+i];
				...
				a[i*nc+i]=f-g;
				for (j=l;j<=n;j++) {
					for (s=0.0,k=i;k<=m;k++) s += a[k*nc+i]*a[k*nc+j];
					f=s/h;
					for (k=i;k<=m;k++) a[k*nc+j] += f*a[k*nc+i];
				}
				for (k=i;k<=m;k++) a[k*nc+i] *= scale;
			}
		}
		w[i]=scale *g;
		g=s=scale=0.0;
		if (i <= m && i != n) {
			for (k=l;k<=n;k++) scale += fabs(a[i*nc+k]);
			if (scale) {
				for (k=l;k<=n;k++) {
					a[i*nc+k] /= scale;
					s += a[i*nc+k]*a[i*nc+k];
				...
				h=f*g-s;
				a[i*nc+l]=f-g;
				for (k=l;k<=n;k++) rv1[k]=a[i*nc+k]/h;
				for (j=l;j<=m;j++) {
					for (s=0.0,k=l;k<=n;k++) s += a[j*nc+k]*a[i*nc+k];
					for (k=l;k<=n;k++) a[j*nc+k] += s*rv1[k];
				}
				for (k=l;k<=n;k++) a[i*nc+k] *= scale;
			}
		}
	...
	for (i=n;i>=1;i--) {
		if (i < n) {
			if (g) {
				for (j=l;j<=n;j++) v[j*nc+i]=(a[i*nc+j]/a[i*nc+l])/g;
				for (j=l;j<=n;j++) {
					for (s=0.0,k=l;k<=n;k++) s += a[i*nc+k]*v[k*nc+j];
					for (k=l;k<=n;k++) v[k*nc+j] += s*v[k*nc+i];
				}
			}
			for (j=l;j<=n;j++) v[i*nc+j]=v[j*nc+i]=0.0;
		}
		v[i*nc+i]=(double)1.0;
		g=rv1[i];
		l=i;
	...
	for (i = IMIN(m,n); i>=1; i--) {
		l=i+1;
		g=w[i];
		for (j=l;j<=n;j++) a[i*nc+j]=0.0;
		if (g) {
			g=(double)1.0/g;
			for (j=l;j<=n;j++) {
				for (s=0.0,k=l;k<=m;k++) s += a[k*nc+i]*a[k*nc+j];
				f = (s/a[i*nc+i])*g;
				for (k=i;k<=m;k++) a[k*nc+j] += f*a[k*nc+i];
			}
			for (j=i;j<=m;j++) a[j*nc+i] *= g;
		} else for (j=i;j<=m;j++) a[j*nc+i]=0.0;
		++a[i*nc+i];
	}
	...
	for (k=n;k>=1;k--) {
		for (its=1;its<=30;its++) {
			...
			if (l == k) {
				if (z < 0.0) {
					w[k] = -z;
					for (j=1;j<=n;j++) v[j*nc+k] = -v[j*nc+k];
				}
				break;
			}
			if (its == 30) {
...
int mossSVD(register double *U, register double *W, register double *V, const double *matx, const int M, const int N)
{
	...
	for (i=1; i<=M; i++) memcpy(&(nrU[i*nc+1]), matx + N*(i-1), N*sizeof(double));
	...
	for (i=1; i <= M; i++) memcpy(U + N*(i-1), &(nrU[i*nc+1]), N*sizeof(double));
	memcpy(W, &(nrW[1]), N*sizeof(double));
	for (i=1; i <= N; i++) memcpy(V + N*(i-1), &(nrV[i*nc+1]), N*sizeof(double));
