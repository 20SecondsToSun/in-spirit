 ...
 unsigned int depth)
{
  ...
  for (d = 0 ; d < forest->dimension ; ++ d) {
    ...
    for (i = dataBegin ; i < dataEnd ; ++ i) {
      ...
      double datum = ((double const*)forest->data)[di * forest->dimension + d];
      mean += datum ;
  ...
  for (i = dataBegin ; i < dataEnd ; ++ i) {
    int di = tree->dataIndex [i] .index ;
    double datum = ((double const*)forest->data)[di * forest->dimension + splitDimension->dimension] ;
...
vl_kdforest_new ( vl_size dimension, vl_size numTrees)
{
  VlKDForest * self = (VlKDForest*)malloc (sizeof(VlKDForest)) ;

  assert(dimension >= 1) ;
  assert(numTrees >= 1) ;
  
  VlRand rnd_gen;
  vl_rand_init(&rnd_gen);
  
  self -> rand = &rnd_gen;//vl_get_rand () ;
  self -> dataType = VL_TYPE_DOUBLE ;
  self -> numData = 0 ;
  self -> data = 0 ;
  self -> dimension = dimension ;
  self -> numTrees = numTrees ;
  self -> trees = 0 ;
  self -> thresholdingMethod = VL_KDTREE_MEAN;//VL_KDTREE_MEDIAN ;
  ...
  //self -> distanceFunction = (void(*)(void))vec_compare_function;
...
{
  vl_uindex ti ;
  if (self->searchIdBook) free (self->searchIdBook) ;
  if (self->trees) {
    for (ti = 0 ; ti < self->numTrees ; ++ ti) {
      if (self->trees[ti]) {
        if (self->trees[ti]->nodes) free (self->trees[ti]->nodes) ;
        if (self->trees[ti]->dataIndex) free (self->trees[ti]->dataIndex) ;
      }
    }
    free (self->trees) ;
  }
...
vl_kdforest_build (VlKDForest * self, vl_size numData, void const * data)
{
  ...
  self->trees = (VlKDTree**)malloc(sizeof(VlKDTree*) * self->numTrees) ;

  for (ti = 0 ; ti < self->numTrees ; ++ ti) {
    self->trees[ti] = (VlKDTree*)malloc (sizeof(VlKDTree)) ;
    self->trees[ti]->dataIndex = (VlKDTreeDataIndexEntry*)malloc (sizeof(VlKDTreeDataIndexEntry) * self->numData) ;
    ...
    self->trees[ti]->nodes = (VlKDTreeNode*)malloc (sizeof(VlKDTreeNode) * self->trees[ti]->numAllocatedNodes) ;
  ...
  if (node->lowerChild < 0) {
    ...
    for (iter = begin ;
      ...
      /* compare the query to this point */
       //dist = self->distanceFunction(self->dimension, ((double const *)query), ((double const*)self->data) + di * self->dimension);
	   dist = vec_compare_function(self->dimension, ((double const *)query), ((double const*)self->data) + di * self->dimension);
                   ...
                   void const * query)
{
  ...
  if (! self -> searchHeapArray) {
    ...
    self -> searchHeapArray = (VlKDForestSearchState*)malloc (sizeof(VlKDForestSearchState) * maxNumNodes) ;
    self -> searchIdBook = (vl_uindex*)calloc (sizeof(vl_uindex), self->numData) ;

    for (ti = 0 ; ti < self->numTrees ; ++ti) {
      double * searchBounds = (double*)malloc(sizeof(double) * 2 * self->dimension) ;
...
inline void
vl_kdforest_set_thresholding_method (VlKDForest * self, int method)
{
...
inline double
vec_compare_function(vl_size dimension, double const * X, double const * Y)
{
  double const *X_end = X + dimension;
  double const *X_end2 = X_end - 3;
