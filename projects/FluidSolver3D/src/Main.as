package
{
	...
	public class Main extends Sprite
	{
		...
		protected function computeSpectrum():void 
		{
            ...
            var i:int = -1;
            var _loc_6:Number = 0;
            var forceMultiplier:Number = 15.0;
            var _loc_2:Number = 0;
            var _loc_3:Number = 0;
            var _loc_4:Number = 0;
            ...
            var _loc_5:Number = int(_loc_2 + _loc_3 + _loc_4) / 3;
            
            if (_loc_5 < 10 && Math.random() < 0.2)
            {
                spinDir *= -1;
            }
            
            solver.fadeSpeed = 1 - Math.min(1.001, _loc_5 / 7);
            solver.solverIter = Math.max(2, Math.min(5, _loc_5));
            
            fx = Math.sin(angle) * _loc_2 * forceMultiplier;
            fy = (-Math.cos(angle)) * _loc_3 * forceMultiplier;
            fz = Math.sin(angle) * Math.cos(angle) * ((_loc_2 + _loc_3) * 0.5) * forceMultiplier;
            
            //trace(fx, fy, fz);
            
            angle += _loc_3 / 100 * spinDir;
        }
