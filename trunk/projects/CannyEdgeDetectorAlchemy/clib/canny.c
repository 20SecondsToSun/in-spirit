...
int performDetection(register short *dx, register short *dy, register int *magn,
                       const long width, const long height, const int thresh,
                       const int thresh2, int *output);
int ByteGradientFull(int *img, short *dx, short *dy, int *mag, int *max_mag);
...
AS3_Val setupCanny(void* self, AS3_Val args)
{
...
AS3_Val destroyCanny(void* self, AS3_Val args)
{
	free( image );
	free( edges );
	free( dx );
...
AS3_Val getImagePointer(void* self, AS3_Val args)
{
	return AS3_Ptr(image);
}
AS3_Val getEdgesPointer(void* self, AS3_Val args)
{
	return AS3_Ptr(edges);
}
AS3_Val setLowThreshold(void* self, AS3_Val args)
{
	AS3_ArrayValue(args, "DoubleType", &thresh_low);

	return 0;
}
AS3_Val setHighThreshold(void* self, AS3_Val args)
{
	AS3_ArrayValue(args, "DoubleType", &thresh_high);

	return 0;
}

AS3_Val runCanny(void* self, AS3_Val args)
{
...
int main()
{
	...
	AS3_Val getImagePointer_m = AS3_Function( NULL, getImagePointer );
	AS3_Val getEdgesPointer_m = AS3_Function( NULL, getEdgesPointer );
	AS3_Val setLowThreshold_m = AS3_Function( NULL, setLowThreshold );
	AS3_Val setHighThreshold_m = AS3_Function( NULL, setHighThreshold );
	AS3_Val destroyCanny_m = AS3_Function( NULL, destroyCanny );

	AS3_Val result = AS3_Object(
						"runCanny: AS3ValType, setupCanny: AS3ValType, getImagePointer: AS3ValType, getEdgesPointer: AS3ValType, setLowThreshold: AS3ValType, setHighThreshold: AS3ValType, destroyCanny: AS3ValType",
						runCanny_m, setupCanny_m, getImagePointer_m, getEdgesPointer_m, setLowThreshold_m, setHighThreshold_m, destroyCanny_m);

	AS3_Release( runCanny_m );
	AS3_Release( setupCanny_m );
	AS3_Release( getImagePointer_m );
	AS3_Release( getEdgesPointer_m );
	AS3_Release( setLowThreshold_m );
	AS3_Release( setHighThreshold_m );
