package ru.inspirit.image
{
	...
	{		
		...
		public const imgRefine:AutoImageProcessor = new AutoImageProcessor();
		...
		public function updateBitmaps(desaturate:Boolean = false, smoothFilter:int = 0, levelsToUpdate:int = 0, refine:Boolean = false):void
		{
			...
			if(desaturate)
			{
				if(refine)
				{
					imgRefine.preProcess(srcImage, bmp);
				} else {
		...
		public function updateMem(desaturate:Boolean = false, smoothFilter:int = 0, levelsToUpdate:int = 0, refine:Boolean = false):void
		{
			var bmp:BitmapData = images[0];
